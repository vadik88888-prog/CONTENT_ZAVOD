...
Процент долгой обработки не симулируется. Для отдельной загрузки ссылки service
получает от yt-dlp реальные процент, скорость, переданный/ожидаемый объём и ETA;
отмена завершает отдельный процесс. Widgets не читают engine JSON напрямую.
