...
def test_moments_keep_quality_risks_selectable_but_block_invalid_source_mapping(
    tmp_path: Path, monkeypatch,
) -> None:
    existing = QCoreApplication.instance()
    if existing is not None and not isinstance(existing, QApplication):
    ...
    warning_id = "candidate-quality-warning"
    invalid_mapping_id = "candidate-invalid-mapping"
    legacy_id = "candidate-legacy"
    analysis["candidates"] = [
        *eligible_candidates,
        {
            "candidate_id": warning_id, "title": "Quality warning", "start_seconds": 20.0,
        ...
        {
            "candidate_id": invalid_mapping_id, "title": "Invalid mapping", "start_seconds": 30.0,
            "end_seconds": 30.0, "potential": "high", "confidence": 0.99,
    ...
    project.candidate_states = {
        candidate_id: "analyzed" for candidate_id in [*candidate_ids, warning_id, invalid_mapping_id, legacy_id]
    }
    services.projects.save(project)
    viewmodel = ProjectViewModel(services)
    screen = ProjectScreen(viewmodel)
    ...
    try:
        screen.open(project)
        app.processEvents()
        screen._toggle_candidate_selection(warning_id)
        assert viewmodel.project is not None
        assert viewmodel.project.review_selected_candidate_ids == [warning_id]
        screen._view_all_candidates()
        app.processEvents()

        assert set(screen._all_candidates_by_id) == {*candidate_ids, warning_id, invalid_mapping_id, legacy_id}
        assert set(screen._draftable_candidates_by_id) == {*candidate_ids, warning_id, legacy_id}
        assert set(screen._candidate_cards) == {*candidate_ids, warning_id, invalid_mapping_id, legacy_id}
        assert set(screen._candidate_selection_buttons) == {*candidate_ids, warning_id, legacy_id}
        assert screen._candidate_cards[invalid_mapping_id].property("candidateBlocked") is True
        assert screen.findChild(QPushButton, f"blocked-candidate-{invalid_mapping_id}") is not None
        recommended_ids = screen._recommended_candidate_ids()
        assert recommended_ids == [*candidate_ids[:3], warning_id, legacy_id]

        screen._select_all_candidates()
        app.processEvents()

        assert viewmodel.project is not None
        assert viewmodel.project.review_selected_candidate_ids == [*candidate_ids, warning_id, legacy_id]

        retries: list[list[str]] = []
        monkeypatch.setattr(viewmodel, "build_drafts", lambda ids: retries.append(list(ids)))
        viewmodel.project.review_selected_candidate_ids = [warning_id]
        viewmodel.project.candidate_states[warning_id] = "draft_failed"
        screen._retry_draft(warning_id)
        assert retries == [[warning_id]]
    finally:
        screen.close()
...
def test_moments_keep_quality_risks_selectable_at_catalogue_scale(
    tmp_path: Path, monkeypatch,
) -> None:
    existing = QCoreApplication.instance()
    if existing is not None and not isinstance(existing, QApplication):
